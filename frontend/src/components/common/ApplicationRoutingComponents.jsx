...
// Компонент для создания условного роутинга
export const ConditionalRoutingBuilder = ({ 
	fields, 
	routes, 
	setRoutes, 
	users, 
	...
	const updateRule = (id, field, value) => {
		setRoutes(routes.map(rule => 
			rule.id === id ? { ...rule, [field]: value } : rule
		))
	}

	const removeRule = (id) => {
		setRoutes(routes.filter(rule => rule.id !== id))
	}

	const getFieldOptions = () => {
		return fields?.filter(field => ['select', 'radio', 'checkbox'].includes(field.type))
	...
	return (
		<div className={`space-y-4 ${className}`}>
			<div className="flex items-center justify-between">
				<div>
					<h4 className="text-sm font-medium text-gray-900">Условное маршрутизирование</h4>
					<p className="text-xs text-gray-600">Назначение исполнителей на основе выбранных значений полей</p>
				</div>
				<button
					type="button"
					onClick={addRule}
			...
			<div className="space-y-3">
				{routes.map(rule => (
					<ConditionalRuleEditor
						...
						valueOptions={getValueOptions(rule.field)}
						users={users}
						roles={roles}
						departments={departments}
				...
				{routes.length === 0 && (
					<div className="text-center py-8 text-gray-500 border-2 border-dashed border-gray-200 rounded-lg">
						<Icon name="git-fork" className="w-12 h-12 mx-auto mb-3 text-gray-300" />
						<p>Добавьте правила условного роутинга</p>
						<p className="text-xs">Правила позволяют назначать разных исполнителей в зависимости от значений полей формы</p>
					</div>
				)}
			</div>
		</div>
...
// Компонент для редактирования условного правила
const ConditionalRuleEditor = ({ rule, fieldOptions, valueOptions, users, roles, departments, onUpdate, onRemove }) => {
	const addTarget = () => {
		const newTarget = {
			id: Date.now(),
			type: 'user',
		...
		onUpdate('targets', [...(rule.targets || []), newTarget])
	}

	const updateTarget = (targetId, field, value) => {
		const updatedTargets = rule.targets.map(target => 
			target.id === targetId ? { ...target, [field]: value } : target
		)
		onUpdate('targets', updatedTargets)
	}

	const removeTarget = (targetId) => {
		onUpdate('targets', rule.targets.filter(target => target.id !== targetId))
	}
	...
	return (
		<div className="p-4 border border-gray-200 rounded-lg bg-white">
			<div className="flex items-center justify-between mb-4">
				<div className="flex items-center space-x-2">
					<Icon name="git-fork" className="w-4 h-4 text-gray-500" />
					<span className="text-sm font-medium text-gray-900">Условное правило</span>
				</div>
				<button
					type="button"
					onClick={onRemove}
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
				<div>
					...
					<select
						...
						onChange={(e) => {
							...
							onUpdate('value', '') // Сбрасываем значение
						}}
						className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#770002] focus:border-transparent"
					>
				...
				<div>
					<label className="block text-xs font-medium text-gray-600 mb-1">Условие</label>
					<select
			...
			<div>
				<div className="flex items-center justify-between mb-3">
					<label className="text-xs font-medium text-gray-600">Цели назначения</label>
					<button
						type="button"
						onClick={addTarget}
				...
				<div className="space-y-2">
					...
					{(!rule.targets || rule.targets.length === 0) && (
						<div className="text-center py-4 text-gray-400 border border-dashed border-gray-200 rounded">
							<p className="text-xs">Добавьте цели для назначения</p>
						</div>
					)}
				</div>
			</div>
...
export const RoutingPreview = ({ routing, conditionalRoutes, users, roles, departments, className = '' }) => {
	...
	return (
		<div className={`p-4 bg-gray-50 rounded-lg ${className}`}>
			...
			{conditionalRoutes?.length > 0 && (
				<div>
					<h5 className="text-xs font-medium text-gray-700 mb-2">Условный роутинг</h5>
					<div className="space-y-2">
						{conditionalRoutes.map(rule => (
							<div key={rule.id} className="bg-white p-3 rounded border">
								<div className="text-xs text-gray-600 mb-2">
									<strong>Если</strong> {rule.field} {rule.condition === 'equals' ? '=' : rule.condition === 'not_equals' ? '≠' : '⊃'} "{rule.value}"
								</div>
			...
			{!routing && (!conditionalRoutes || conditionalRoutes.length === 0) && (
